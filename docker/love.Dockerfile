...
RUN apt-get update && apt-get install -y git build-essential libraspberrypi-bin libraspberrypi-dev debhelper dh-autoreconf pkg-config libtool g++ libfreetype6-dev luajit libluajit-5.1-dev libmodplug-dev libmpg123-dev libopenal-dev libphysfs-dev libsdl2-dev libogg-dev libvorbis-dev libtheora-dev zlib1g-dev && apt-get build-dep -y libsdl2
...
CMD cd /usr/src/love && ./platform/unix/automagic | tee /work/buildlog-love.txt && ./configure | tee -a /work/buildlog-love.txt && cp -R platform/unix/debian/ . && dpkg-buildpackage -us -uc -j12 | tee -a /work/buildlog-love.txt && cp /usr/src/*.deb /work